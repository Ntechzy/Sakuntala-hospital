        ...
        >
          {navbarData.map((item, index) => (
            <li key={index} className="relative group">
              ...
              {item.submenu && (
                <ul
                  className={`absolute left-0 mt-2 w-48 bg-white text-[#A83679] rounded-md shadow-lg opacity-0 group-hover:opacity-100 ${
                ...
                >
                  {item.submenu.map((submenuItem, submenuIndex) => (
                    <li key={submenuIndex}>
                      <Link
                        to={submenuItem.link}
                        onClick={() => setIsopened(!Isopened)} // Close the menu when submenu item is clicked
                        className="block px-4 py-2 text-sm hover:bg-gray-100 bg-slate-600 hover:text-[#A83679]"
                      >
                        {submenuItem.label}
                      </Link>
                    </li>
